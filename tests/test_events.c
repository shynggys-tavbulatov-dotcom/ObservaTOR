...
int main(void)
{
    ...
    if (!contains_event(events, count, EVENT_BOOTSTRAP_COMPLETE)) {
        puts("FAIL: bootstrap complete");
        return 1;
    }

    if (!contains_event(events, count, EVENT_BOOTSTRAP_PROGRESS)) {
    ...
    if (!contains_event(events, count, EVENT_CONNECTION_FAILURE)) {
        puts("FAIL: connection failure");
        return 1;
    }

    if (!contains_event(events, count, EVENT_ONION_SERVICE)) {
